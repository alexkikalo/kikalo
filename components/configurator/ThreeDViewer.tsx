...
export function ThreeDViewer({ variant, className = '' }: ThreeDViewerProps) {
  ...
  return (
    <div className={`relative w-full overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-950 shadow-2xl ${className}`}>
      <div className="relative aspect-[16/10] md:aspect-[4/3] lg:aspect-square">
        <Canvas
          key={key}
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent p-4 md:p-5">
          <div className="flex items-center justify-between gap-3 text-xs">
            ...
            <div className="flex items-center gap-1.5">
              <button onClick={() => setAutoRotate(!autoRotate)} className={`flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium transition-all active:scale-[0.985] ${autoRotate ? 'border-zinc-700 bg-zinc-900 text-white' : 'border-zinc-800 bg-zinc-950 text-zinc-400 hover:text-white'}`}>
                <RotateCw className={`h-3.5 w-3.5 ${autoRotate ? 'animate-spin [animation-duration:2.2s]' : ''}`} />
                <span className="hidden sm:inline">Auto</span>
              </button>
              <button onClick={resetView} className="flex items-center gap-1.5 rounded-full border border-zinc-800 bg-zinc-950 px-3 py-1.5 text-xs font-medium text-zinc-400 transition-all hover:text-white active:scale-[0.985]}">
        ...
        <div className="absolute left-4 top-4 rounded-full bg-black/60 px-3 py-1 text-[10px] font-mono tracking-[1.5px] text-zinc-400 backdrop-blur">REAL-TIME PREVIEW</div>
        <div className="absolute right-4 top-4 rounded-xl border border-white/10 bg-black/60 px-3 py-2 text-right text-xs backdrop-blur">
          ...
          <div className="font-medium text-white tabular-nums">{width} × {depth} × {height} <span className="text-[10px] text-zinc-400">mm</span></div>
        </div>
      </div>
    </div>
  )
